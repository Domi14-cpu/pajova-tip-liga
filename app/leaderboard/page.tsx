...
function LeaderboardContent() {
  ...
  useEffect(() => {
    async function loadLeaderboard() {
      ...
      if (error) {
        setMessage(
          `Žebříček se nepodařilo načíst: ${error.message}`
        );
      } else {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-6xl px-5 py-14 pb-28 lg:pb-14">
      <section className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-zinc-900 to-black p-7 sm:p-10">
        ...
        <p className="mt-4 max-w-2xl leading-7 text-zinc-400">
          Pořadí je počítané jen z tipů v právě vybrané sezóně.
        </p>
        ...
        <label className="mt-7 block max-w-xl">
          ...
          <select
            ...
            className="w-full rounded-xl border border-white/10 bg-black px-4 py-3.5 font-black outline-none focus:border-amber-400"
          >
            {seasons.map((season) => (
        ...
        {currentSeason?.is_private && (
          <p className="mt-4 text-sm font-bold text-amber-300">
            🔒 Zobrazuješ zamknutou sezónu, do které máš přístup.
          </p>
        )}
      </section>
      ...
      <section className="mt-8 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.035]">
        {loading ? (
          ...
          <p className="p-8 text-center text-zinc-400">
            V této sezóně zatím nikdo netipoval.
          </p>
        ) : (
          players.map((player, index) => (
            ...
            >
              ...
              <div className="flex min-w-0 items-center gap-3">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full bg-amber-400 font-black text-black">
                  {player.avatar_url ? (
                    <img
                ...
                <span className="truncate font-black group-hover:text-amber-400">
                  {player.nickname}
                </span>
              </div>
...
export default function LeaderboardPage() {
  return (
    <Suspense
      fallback={
        <main className="mx-auto min-h-screen w-full max-w-6xl px-5 py-14 pb-28 lg:pb-14">
          <section className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-zinc-900 to-black p-10">
